...
def train_one_epoch(model: torch.nn.Module, criterion: torch.nn.Module,
    ...
    for i in metric_logger.log_every(range(len(data_loader)), print_freq, header):
        ...
        assert isinstance(outputs, (list, tuple)) and isinstance(targets, (list, tuple))

        loss_dict_cur_frame = criterion(outputs[0], [targets[0]])
        weight_dict = criterion.weight_dict
        losses_cur_frame = sum(
            loss_dict_cur_frame[k] * weight_dict[k] for k in loss_dict_cur_frame.keys() if k in weight_dict
        )
        losses = losses_cur_frame
        for j, temporal_output in enumerate(outputs[1:]):
            loss_dict_temporal = criterion(temporal_output, [targets[j + 1]])
            losses_temporal = sum(
                loss_dict_temporal[k] * weight_dict[k] for k in loss_dict_temporal.keys() if k in weight_dict
            )
            losses = losses + losses_temporal
        loss_dict = loss_dict_cur_frame
...
def evaluate(model, criterion, postprocessors, data_loader, base_ds, device, output_dir):
    ...
    for samples, targets in tqdm(data_loader):
        ...
        for output, target in zip(outputs[:3], targets[:3]):
            target = [target]
