...
def crop(image, target, region):
    if isinstance(image, list):
        cropped_image = [F.crop(i, *region) for i in image]
    else:
...
def resize(image, target, size, max_size=None):
    ...
    if isinstance(image, list):
        ...
        rescaled_image = [F.resize(i, size) for i in image]
    else:
    ...
    if isinstance(image, list):
        ratios = tuple(
            ...
            for s, s_orig in zip(rescaled_image[0].size, image[0].size))
    else:
        ratios = tuple(
            float(s) / float(s_orig)
...
class RandomSizeCrop(object):
    ...
    def __call__(self, img: PIL.Image.Image, target: dict):
        if isinstance(img, list):
            ...
            h = random.randint(self.min_size, min(img[0].height,
                                                  self.max_size))
...
class ToTensor(object):
    def __call__(self, img, target):
        if isinstance(img, list):
            img = [F.to_tensor(i) for i in img]
...
class Normalize(object):
    ...
    def __call__(self, image, target=None):
        image = torch.stack([F.normalize(image[i], mean=self.mean, std=self.std) for i in range(image.shape[0])])
