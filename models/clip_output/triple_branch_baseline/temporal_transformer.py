...
class TemporalTransformer(DeformableTransformer):
    def __init__(
        ...
        # self.reference_points = nn.ModuleList([deepcopy(self.reference_points) for _ in range(self.num_frames)])
    ...
    def forward(
        ...
        query_embeds, reference_points, tgt = [], [], []
        # for query_embed_i, reference_points_layer in zip(query_embed, self.reference_points):
        for query_embed_i in query_embed:
            query_embed_i, tgt_i = torch.split(query_embed_i, channels, dim=1)
            query_embed_i = query_embed_i.unsqueeze(0)
            tgt_i = tgt_i.unsqueeze(0)
            reference_points_i = self.reference_points(query_embed_i).sigmoid()
            query_embeds.append(query_embed_i)
            reference_points.append(reference_points_i)
            tgt.append(tgt_i)
        query_embed = torch.cat(query_embeds, dim=0)  # 3, 300, 256
        reference_points = torch.cat(reference_points, dim=0)  # 3, 300, 2
        tgt = torch.cat(tgt, dim=0)  # 3, 300, 256
