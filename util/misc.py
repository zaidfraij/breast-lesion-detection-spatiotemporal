...
def collate_fn_for_clip(batch):
    batch = list(zip(*batch))
    batch[0] = nested_tensor_from_tensor_list(batch[0])
    # batch[1] is either a list of dicts or a list of lists, each of which contains dicts, so we need to convert it to a list of dicts
    if isinstance(batch[1][0], list):
        batch[1] = batch[1][0]
    return tuple(batch)